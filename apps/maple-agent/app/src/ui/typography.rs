...
pub enum ChatFontFamily {
    ...
    Geist,
    /// Georgia. Stock macOS has it; New York and named SF Pro Text do not.
    Serif,
}
...
const FAMILY_MANROPE: u8 = 1;
const FAMILY_GEIST: u8 = 2;
const FAMILY_SERIF: u8 = 3;

static FAMILY: AtomicU8 = AtomicU8::new(FAMILY_SYSTEM);
static SIZE: AtomicU8 = AtomicU8::new(DEFAULT_CHAT_FONT_SIZE);

/// Serializes live typography writes in tests. Chat tests that call
/// `apply_defaults` must take this too: that path stores these atomics.
#[cfg(test)]
pub(crate) static APPLY_LOCK: parking_lot::Mutex<()> = parking_lot::Mutex::new(());

impl ChatFontFamily {
    pub const ALL: [Self; 4] = [Self::System, Self::Manrope, Self::Geist, Self::Serif];

    /// Unknown values, including the retired `"sf-pro"` picker id, fall
    /// back to System. `.SystemUIFont` already is SF Pro on macOS.
...
/// Apply only the face, leaving size, weight, and style to the caller
/// or the cascade. `Styled::font()` would pin those to Normal.
pub fn with_family<T: Styled>(el: T, family: ChatFontFamily) -> T {
    el.font_family(family.family_name())
}
...
mod tests {
    ...
    fn unknown_family_is_system() {
        ...
        assert_eq!(ChatFontFamily::parse("sf-pro"), ChatFontFamily::System);
