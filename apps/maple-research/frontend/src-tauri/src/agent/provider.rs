...
pub(super) const DEVICE_CLOCK_ERROR_MESSAGE: &str =
    "Maple could not verify the secure server connection because this device's date or time looks wrong. Check the device's date, time and time zone settings, then try again";
