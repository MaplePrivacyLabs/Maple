...
// secret). The recovery reset completion opens the stored wrap through
// `decrypt_seed_v1` with `compute_recovery_auth_binding`; there is no
// separate recovery-wrap key/AAD envelope representation.
